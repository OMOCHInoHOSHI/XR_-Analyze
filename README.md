...
- 固定すると**説明ウィンドウ**が画面中央に開き、対象の説明文(LLM生成)が表示されます
  (下記「固定した物体の AI 説明」参照)。
...
対象の説明文が日本語で表示されます。説明文はローカルLLM (Ollama / `gemma4:e4b`) が
検出ラベルから 2〜3 文で生成します (モデル選定の経緯:
[ADR-0024](docs/adr/0024-llm-object-explanation.md))。

- 初回は生成に**数秒**かかります(ウィンドウには「説明を生成中…」と出ます)。以後、

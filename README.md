...
- ブラウザで `http://127.0.0.1:8100/` → 黒背景に検出枠 + デバッグ映像
  ...
  - `+` `-` でズーム、`↑` `↓` `←` `→` で表示位置を調整 (下記「カメラをスマートグラスに合わせ込む」参照)
...
| `CAM_ZOOM` | 1.0 | デジタルズーム倍率(中央クロップ)。1.0で等倍、1.5なら幅・高さを1/1.5に中央クロップ。1.0未満は無効(1.0にクランプ) |
| `CAM_OFFSET_X` / `CAM_OFFSET_Y` | 0.0 / 0.0 | 切り抜き中心のオフセット。フレーム幅/高さに対する正規化値(0.1=幅の10%右へ、yは+で下へ) |
...
`check_camera` のどちらでも調整できます。両者は別プロセスなので調整中の値は共有されず、
確定値を `CAM_ZOOM` / `CAM_OFFSET_*` としてコピペで引き継ぐ形になります(どちらも
同じ形式で出力します)。
...
`[calib] CAM_ZOOM=... CAM_OFFSET_X=... CAM_OFFSET_Y=...` が出力されるので、
気に入った値をコピーして次回の起動時に付ければ再現できます。

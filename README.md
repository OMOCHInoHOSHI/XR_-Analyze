...
トップページ(`/`)で `m` を押してから数字キーで鑑定法を切り替えます
([ADR-0015](docs/adr/0015-key-binding-policy.md))。`m` の後に3秒放置するか、
`1`/`2` 以外を押すと取り消されます (そのキー本来の機能も動きません)。
...
- 枠を出す1件は**画面中央の照準で狙っているもの**です。XRグラスでは頭を向けて対象を
  狙うため、画面中央 = 見ているものとして扱います。判定は次の順です
...
- 固定中は簡易追従で対象を引き継ぎます ([ADR-0012](docs/adr/0012-lock-tracking-by-iou.md))。
...
既定は `六: 蒼環の菱標` です。`m` を先に押した場合はモード切替が優先されます。
`0`(調整のリセット)は従来どおりです。

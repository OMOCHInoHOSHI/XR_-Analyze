...
- ブラウザで `http://127.0.0.1:8100/` → 黒背景に検出枠 + デバッグ映像
  ...
  - `m` → `1` / `2` で検出モードを切り替え (下記「検出モードの切り替え」参照)
...
トップページ(`/`)で `m` を押してから数字キーでモードを切り替えます。数字キーは
...
| キー | モード | 表示 |
|---|---|---|
| `m` → `1` | シングルターゲット | 常に**1件だけ**枠を出す。`Enter` でフォーカス固定 |
| `m` → `2` | マルチターゲット (既定) | 信頼度の高い順に**最大10件** |

### シングルターゲットモードとフォーカス固定

- 枠を出す1件は**画面中央に最も近い検出**です (XRグラスでは頭を向けて対象を狙うため、
  画面中央 = 見ているもの)。同距離なら信頼度の高い方を選びます。
- `Enter` を押すと、その瞬間に枠が出ている対象にフォーカスを固定します(枠が黄色になり
  ラベルに `[固定]` が付く)。固定中は頭を動かして別の物体が中央に来ても対象は変わりません。
- 解除は `Enter` をもう一度押すか `Esc`。モードを切り替えたときも解除されます。
...
> モード切替とフォーカス固定はブラウザ側の表示ポリシーで、推論内容や `/ws` の出力は
...
| キー | 意匠 | 内容 |
|---|---|---|
| `1` | 壱: 蒼の走査 (既定) | 四隅の鉤括弧だけを光らせ、枠内を走査線が上下する。固定で走査が止まり内側に確定枠 |
...
| `6` | 六: 蒼環の菱標 | `伍` の構図に `壱` の配色と菱形のあしらいを載せた組み合わせ。環の上・左・下と引き出し線の起点に菱形を置き、明滅させる。固定で明滅が止まり菱形が開く |
...
`DEBUG_THEME_KEYS` を `false` にするだけでも、`data-theme` が付かなくなるので
見た目は「肆: 素の緑」に戻ります。
...
> 巻き戻り、まったく進まなくなるためです。
> モード切替UI(HUDの見せ方や呼称)のファンタジー化は未着手で、現状のままです。

...
- ブラウザで `http://127.0.0.1:8100/` → 黒背景に検出枠 + デバッグ映像
  ...
  - `↑` `↓` `←` `→` で単体鑑定の対象を選び替え、`Enter` で鑑定を固定
    (`→`/`←` は x が次のもの、`↑`/`↓` は y が次のものへ。押し続けると視界の
    検出を順に巡り、端まで行くと反対の端へ一周して戻ります。下記「単体鑑定・
    対象の選び替え・鑑定の固定」参照)
...
| `m` → `3` (または矢印+Enter、`c` の単独キーも同じ) | 調整 | ズーム・切り抜き位置・保存 (下記「カメラをスマートグラスに合わせ込む」参照) |
...
- **矢印キー (`↑↓←→`)** は、今の対象 (無ければ照準) を起点に**軸に沿って
  並べた輪**を回します。`→`/`←` は検出の中心 `x` だけを、`↑`/`↓` は中心 `y`
  だけを見て、「その向きで軸の位置が最も手前のもの」へ対象を移します。
  「最も近い検出」ではなく「x (または y) が次の検出」を選ぶ点に注意してください。
  半径による足切りは矢印には掛かりません。
  押し続けると視界の検出を軸ごとに順に巡り、その向きの端まで着くと**反対の端へ
  一周して戻ります**(このとき報せに `(一周)` が付きます)。軸の位置が起点と
  厳密に同じ検出(例: 真上・真下に重なった物)はその軸では次に選ばれません。
  `↑`/`↓` と `←`/`→` を使い分けて拾ってください。軸上に他の候補が無ければ
  `その向きに対象がありません` と出て何も起きません
  ([ADR-0022](docs/adr/0022-arrow-navigation-wraps.md))。
- 対象の状態は **照準ベース → 選択中 → 固定** の3段です。矢印で選ぶと「選択中」
  (枠の外側に破線+ラベル `[選択]`)になり、`Enter` で「固定」(枠が金色+ラベル
  `[固定]`)になります。固定中に矢印を押すと選択中へ降格したうえで次へ移ります
  (移動が成立しなければ降格せず、固定のまま留まります)。
...
| 鑑定法 | `単体鑑定` または `広域鑑定` |
| 調整モードのキー案内 | `c` または `m` → `3` で調整モードに入っている間だけ開く。切り抜き位置・ズーム・保存・モードへ戻る・終了のキー一覧 |

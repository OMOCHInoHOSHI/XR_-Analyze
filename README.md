...
> 中心からの距離は**表示ピクセル**で測ります。正規化座標(0-1)のまま
> `dx*dx + dy*dy` で測ると、16:9 では x の1単位=幅・y の1単位=高さのため
> 横方向のズレが 1.78 倍過小評価され、画面端の物体が中央脇の物体に勝ちます
> (実測: 端560px の物体が中央脇320px の物体に勝つ)。
...
ないと上下または左右にレターボックスの帯が付きます。枠は stage 全体ではなく
**映像が実際に占める矩形**を基準に配置します (帯を考慮しないと、1440x900 のウィンドウで
最大36px、正方形に近いウィンドウで175px ずれます)。映像OFF(AR表示)でも同じ矩形を
使います。実世界に対する枠の縦横比を保つためです。矩形は `/ws` の `source_size` から
求めるので、追加の通信は要りません。
